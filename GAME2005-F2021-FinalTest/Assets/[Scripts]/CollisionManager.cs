...
public class CollisionManager : MonoBehaviour
{
    ...
    public static void ApplyVelocity(RigidBody3D objectA, RigidBody3D objectB, Vector3 normal)
    {
        //find relative velocity of B relative to A
        Vector3 relativeVelocityAB = objectB.velocity - objectA.velocity;
        ...
        float restitution = (objectA.elasticity + objectB.elasticity) * 0.5f;
        float deltaV = -(relativeNormalVelocityAB * (1.0f + restitution) * 2.0f);
        float impulse;

        if (objectA.bodyType == BodyType.STATIC && objectB.bodyType == BodyType.DYNAMIC)
        {
            impulse = deltaV * objectB.mass;
            objectB.velocity += normal * impulse / objectB.mass;
        }
        else if (objectA.bodyType == BodyType.DYNAMIC && objectB.bodyType == BodyType.STATIC)
        {
            impulse = deltaV * objectA.mass;
            objectA.velocity -= normal * impulse / objectA.mass;
        }
        else if (objectA.bodyType == BodyType.DYNAMIC && objectB.bodyType == BodyType.DYNAMIC)
        {
            impulse = deltaV / ((1.0f / objectA.mass) + (1.0f / objectB.mass));
